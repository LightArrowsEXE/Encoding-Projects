...
def generate_comparison_NCOP1(src: FileInfo, enc: Union[os.PathLike[str], str], flt: vs.VideoNode) -> None:
    """For the NCOP because of weird fuckery"""
    make_comps(
        {
            'source': src.clip_cut + src.clip_cut[-1] * 9,
            'filtered': flt,
            'encode': source(str(enc), force_lsmas=True, cachedir='')
        },
        num=int(src.clip_cut.num_frames / 700) if src.clip_cut.num_frames > 5000 else 50,
        collection_name=f'[Kaleido-subs] {src.name} Encode test',
        path=f'.comps/{src.name}', force_bt709=True, slowpics=True, public=False
    )
...
class Encoder_NCOP1:
    """"Simple duplicate Encoder Class for NCOP1 because we need another FileInfo and I'm lazy"""
    def __init__(self, file: FileInfo, clip: vs.VideoNode, audio_file: FileInfo) -> None:
        self.file = file
        self.clip = clip
