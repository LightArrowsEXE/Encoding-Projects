...
# Sources
JP_BD = FileInfo(r'BDMV/[BDMV][アニメ][210127][MAJO_NO_TABITABI_1][Blu-Ray BOX 上]/BDMV/STREAM/00004.m2ts',
...
def pre_corrections() -> Union[vs.VideoNode, Tuple[vs.VideoNode, ...]]:
    ...
    sqmask = lvf.mask.BoundingBox((1397, 370), (86, 74)).get_mask(src_BD).bilateral.Gaussian(sigma=10)
    fixed = lvf.rfs(src_BD, src_BD.std.MaskedMerge(src_WEB, sqmask), [(21248, 21310)])

    # Another face redraw undo — https://slow.pics/c/QoxjoSeB
    sqmask = lvf.mask.BoundingBox((1310, 356), (94, 80)).get_mask(src_BD).bilateral.Gaussian(sigma=10)
    fixed = lvf.rfs(fixed, fixed.std.MaskedMerge(src_WEB, sqmask), [(21447, 21493)])

    # Very minor line fuck-up
    sqmask = lvf.mask.BoundingBox((1320, 478), (8, 14)).get_mask(src_BD)
    fixed = lvf.rfs(fixed, fixed.std.MaskedMerge(src_WEB, sqmask), [(21452, 21454)])
