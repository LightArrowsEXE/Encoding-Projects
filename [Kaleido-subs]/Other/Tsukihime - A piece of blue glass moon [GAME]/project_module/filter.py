...
def rescaler(clip: vs.VideoNode, height: int, shader_file: Optional[str] = None) -> Tuple[vs.VideoNode, vs.VideoNode]:
    """
    ...
    clip_y = get_y(clip)
    descale = Lanczos(taps=5).descale(clip_y, get_w(height, clip.width/clip.height), height)

    if shader_file:
        rescale = fsrcnnx_upscale(descale, shader_file=shader_file, downscaler=Bicubic(b=-1/2, c=1/4).scale)
    else:
    ...
    return depth(scaled, 16), depth(detail_mask, 16, range_in=Range.FULL, range=Range.LIMITED)
...
def generate_comparison(src: FileInfo, enc: vs.VideoNode) -> None:
    ...
    make_comps(
        ...
        num=int(src.num_frames / 500) if src.clip_cut.num_frames > 5000 else 80,
