...
# Sources
JP_BD = FileInfo(r"E:/src/[神的记事本][Heaven's Memo Pad][神様のメモ帳][BDMV]/神様のメモ帳_1/KAMIMEMO_01/BDMV/STREAM/00004.m2ts",
                 (None, -24), idx=lambda x: source(x), preset=[PresetBD, PresetFLAC])
...
def filterchain() -> Union[vs.VideoNode, Tuple[vs.VideoNode, ...]]:
    ...
    l_mask = vdf.mask.FDOG().get_mask(src_y, lthr=0.125, hthr=0.050).rgsf.RemoveGrain(4).rgsf.RemoveGrain(4)
    l_mask = l_mask.std.Minimum().std.Deflate().std.Median().std.Convolution([1] * 9).std.Maximum()

    descaled = lvf.kernels.Mitchell().descale(src_y, 1280, 720)
    rescale_nn3 = vdf.scale.nnedi3_upscale(descaled, use_znedi=True)
    rescale_fsrcnnx = vdf.scale.fsrcnnx_upscale(descaled, downscaler=None, shader_file=shader_file,
                                                upscaled_smooth=rescale_nn3, profile='zastin')
    rescale = core.std.Merge(rescale_nn3, rescale_fsrcnnx)

    downscale = lvf.scale.ssim_downsample(rescale, src.width, src.height, kernel=lvf.kernels.BicubicDidee())
    downscale = core.std.MaskedMerge(src_y, downscale, l_mask)

    scaled = depth(vdf.misc.merge_chroma(downscale, rkt), 16)

    # Denoise
    stab = haf.SMDegrain(scaled, tr=3, thSAD=40, plane=0, Str=3.0)
