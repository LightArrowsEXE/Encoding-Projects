...
def generate_comparison(src: FileInfo, enc: Union[os.PathLike[str], str], flt: vs.VideoNode) -> None:
    ...
    make_comps(
        ...
        num=int(src.clip_cut.num_frames / 500) if src.clip_cut.num_frames > 5000 else 40,
        path=f'.comps/{src.name}_{datetime.now().strftime("%d_%m_%Y-%H_%M_%S")}',
        force_bt709=True,
        slowpics_conf=SlowPicsConf(
            collection_name=f'{src.name} Encode (autogenerated comp)', public=False, remove_after=False
        )
    )
...
class Encoder:
    ...
    def run(self, preset: Optional[Preset] = None, clean_up: bool = True,
            make_comp: bool = False, lossless: bool = False,
            x264: bool = False,
            zones: Optional[Dict[Tuple[int, int], Dict[str, Any]]] = None,
            flac: bool = False, all_tracks: bool = False,
            alt_src: Optional[FileInfo] = None,
            passthrough: bool = False,) -> None:
        """
        :param preset:          Preset for how to handle the encode (currently not implemented)
        ...
        :param alt_src:         Alternative FileInfo object to source the audio from
        :param passthrough:     Whether to passthrough the audio or re-encode it (currently not implemented)
        """
        assert self.file.a_src

        if preset:
            logger.warning("`preset` has not been implemented yet!")
        ...
        audio_files = video_source(self.file.path.to_str() if not alt_src else alt_src.path.to_str(),
        ...
        muxer = Mux(
            self.file,
            streams=(
                VideoStream(self.file.name_clip_output, 'Original BDrip by LightArrowsEXE@Kaleido', JAPANESE),
