...
from vardautomation import (JAPANESE, AudioStream, Chapter, ChapterStream,
                            FFV1Encoder, FileInfo, MatroskaXMLChapters, Mux,
...
def pick_settings(f: FileInfo) -> str:
    """
    There used to be more logic here, but let's not
    make begna mention this every time he runs a script.
    """
    # ep = f.name[-2:]
...
class Encoder:
    ...
    def run(self, clean_up: bool = True, make_comp: bool = True,
        ...
        muxer = Mux(
            self.file,
            streams=(
                VideoStream(self.file.name_clip_output, 'HEVC BDrip by LightArrowsEXE@DameDesuYo', JAPANESE),
