...
# Sources
SRC = vse.FileInfo(f"{ini.bdmv_dir}/[BDMV]HONZUKI/HONZUKI_2/BDMV/STREAM/00006.m2ts", (24, -24))
...
def filterchain(src: vs.VideoNode = SRC.clip_cut,
    ...
    sqmask = core.akarin.Expr([
        ...
        lvf.mask.BoundingBox((391, 348), (3, 4)).get_mask(beige),
        lvf.mask.BoundingBox((492, 197), (3, 3)).get_mask(beige),
        lvf.mask.BoundingBox((573, 240), (3, 2)).get_mask(beige),
        lvf.mask.BoundingBox((1555, 37), (4, 4)).get_mask(beige),
        lvf.mask.BoundingBox((1570, 59), (5, 4)).get_mask(beige),
        lvf.mask.BoundingBox((1886, 151), (2, 2)).get_mask(beige),
        lvf.mask.BoundingBox((254, 441), (2, 3)).get_mask(beige)
    ], "x y z a b c d e max max max max max max max")

    fix_beige = core.std.MaskedMerge(src, beige, sqmask)
    fix_beige = lvf.rfs(src, fix_beige, [(33927, None)])
    ...
    restore_src = core.std.MaskedMerge(depth(grain, 32), fix_beige, credit_mask)
